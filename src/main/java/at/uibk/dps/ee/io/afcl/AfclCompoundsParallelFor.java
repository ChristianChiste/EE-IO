...
public final class AfclCompoundsParallelFor {
	...
	/**
	 * Validates the provided iterator list.
	 * 
	 * @param iterators       the iterator list to validate
	 * @param parallelForName the name of the parallel for
	 */
	protected static void checkIterators(List<String> iterators, String parallelForName) {
		if (iterators.isEmpty()) {
			throw new IllegalArgumentException("No iterators provided for the parallelFor " + parallelForName);
		}
		Predicate<String> numberIteratorPredicate = string -> UtilsAfcl.isSrcString(string)
				|| UtilsIO.readableAsInt(string);
		if (iterators.stream().filter(numberIteratorPredicate).count() > 1) {
			throw new IllegalArgumentException(
					"More than one int iterator provided for parallelFor " + parallelForName);
		}
	}
